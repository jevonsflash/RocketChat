...
namespace RocketChat.Services
{
    internal class ChannelsService : IChannelsService
    {
        ...
        public async Task<Result<Channel>> AddAll(AddAll payload)
        {
        ...
        public async Task<Result<Channel>> Create(Create payload)
        {
        ...
        public async Task<Result<Channel>> Invite(UserAction payload)
        {
            var response = await _restClientService.Post<ChannelResult>(GetUrl("invite"), payload);
            return ProcessChannelResponse(response);
        }

        public async Task<Result<Channel>> Join(Join payload)
        {
            var response = await _restClientService.Post<ChannelResult>(GetUrl("join"), payload);
            return ProcessChannelResponse(response);
        }

        public async Task<Result<Channel>> Kick(UserAction payload)
        {
        ...
        {
            var response = await _restClientService.Get<ChannelsResult>(GetUrl("list"), true);
        ...
        public async Task<Result<Channel>> Rename(Rename payload)
        {
        ...
        public async Task<Result<Channel>> SetCustomFields(SetCustomFields payload)
        {
        ...
        private static Result<IEnumerable<Channel>> ProcessChannelsResponse(ApiResponse<ChannelsResult> response)
        {
            if (response.StatusCode == HttpStatusCode.OK)
            {
                return response.Result.Success
                    ...
                    : new ErrorResult<IEnumerable<Channel>>(response.Message, response.StatusCode);
            }
            else
                return new ErrorResult<IEnumerable<Channel>>(response.Message, response.StatusCode);
        }
        ...
        private static Result<Channel> ProcessChannelResponse(ApiResponse<ChannelResult> response)
        {
            if (response.StatusCode == HttpStatusCode.OK)
            {
                return response.Result.Success
                    ...
                    : new ErrorResult<Channel>(response.Message, response.StatusCode);
            }
            else
                return new ErrorResult<Channel>(response.Message, response.StatusCode);
        }
    }
}